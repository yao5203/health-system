<template>
  <div class="app-shell">
    <header class="site-header">
      <RouterLink class="brand-block" to="/admin/auth">
        <span class="eyebrow">Health System Admin</span>
        <h1>人体健康评测与康养系统 · 管理员端</h1>
      </RouterLink>
      <nav class="switch-group">
        <RouterLink class="switch-btn" to="/admin/auth">管理员登录</RouterLink>
      </nav>
    </header>
    ...
    <main class="route-shell">
      <div v-if="routeError" class="panel app-error-panel">
        ...
        <div class="action-row">
          ...
          <RouterLink class="ghost-btn" to="/admin/auth">返回登录页</RouterLink>
        </div>
      </div>
...
const splashUserType = ref('admin')
...
function playEntrySplashIfNeeded(currentPath) {
  ...
  splashUserName.value = payload.userName || '管理员'
  splashUserType.value = payload.userType || 'admin'
